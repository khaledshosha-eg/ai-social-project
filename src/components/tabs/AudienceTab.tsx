...
const AudienceTab = ({ data }: { data: AudienceData }) => {
  // حماية أولية: لو الداتا مش موجودة أصلاً الصفحة ما تضربش
              ...
              <div className="relative inline-block">
                <svg className="w-32 h-32 transform -rotate-90">
                  ...
                  <circle 
                    ...
                    strokeDashoffset={364.4 - (364.4 * (data.purchase_intent?.percentage || 0)) / 100} 
                    className="text-amber-500" 
                  />
                </svg>
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-3xl font-black text-white">{data.purchase_intent?.percentage || 0}%</span>
          ...
          <Card title="Ideal Customer Persona" icon={Users} color="blue">
            <div className="space-y-6">
              <div className="p-4 rounded-2xl bg-blue-500/5 border border-blue-500/10">
                <p className="text-sm text-white/90 leading-relaxed italic">"{data.persona?.description || 'No description available'}"</p>
              </div>
