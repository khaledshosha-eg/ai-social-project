      ...
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-3">
            <button onClick={handleExport} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-bold transition-all">
              <Download className="w-4 h-4" /><span>Export PDF</span>
            </button>
            <button onClick={handleShare} className="p-2 rounded-lg bg-secondary hover:bg-secondary/80 border border-border transition-all">
        ...
        {data.ranking && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            className="bg-card border border-border p-6 rounded-2xl shadow-xl">
            ...
            <div className="grid grid-cols-3 gap-4 text-center">
              <div className="bg-secondary/30 rounded-xl p-4">
                ...
                <p className="text-xs text-muted-foreground mt-1">مرتبتك الحالية</p>
              </div>
              <div className="bg-secondary/30 rounded-xl p-4">
                <p className="text-sm font-bold text-foreground break-all">{data.ranking.strongest_competitor}</p>
                <p className="text-xs text-muted-foreground mt-1">أقوى منافس</p>
              </div>
              <div className="bg-secondary/30 rounded-xl p-4">
                <p className="text-sm font-bold text-rose-400">{data.ranking.gap_to_leader}</p>
                <p className="text-xs text-muted-foreground mt-1">الفجوة</p>
              </div>
            </div>
          </motion.div>
        )}
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="bg-card/50 border border-border p-6 rounded-2xl shadow-xl">
            <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
        ...
        {data.action_plan && data.action_plan.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            className="bg-card border border-border p-8 rounded-3xl shadow-2xl">
            ...
            <div className="space-y-4">
              {data.action_plan.map((step, i) => (
                <motion.div key={i} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }}
                  ...
                  className="flex gap-4 p-5 rounded-xl bg-secondary/30 border border-border hover:border-primary/30 transition-all">
                  ...
                  <div className="space-y-2 flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">📊 السبب: {step.reason}</p>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <p className="text-lg text-muted-foreground leading-relaxed">{data.analysis_summary}</p>
        </motion.div>
